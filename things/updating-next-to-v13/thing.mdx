...
I guessed this error is from `esbuild` because I often saw the `jsx-runtime.js` file in other projects using `esbuild` as the build output. `mdx-bundler` is only the package using `esbuild` in this blog. So I just updated it to latest, then it fixed the error.
...
When a page moves to another page, users need to wait for the moment. But Next.js using prerendering doesn't make it happen.  
